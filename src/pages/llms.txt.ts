...
export async function GET() {
	...
	const lines = [
		...
		heading(s.work),
		'',
		...doors.map((door) => `- [${door.title}](${abs(door.link!.href)}): ${door.body}`),
		'',
		'### Founders and scale-ups',
		...capabilities.founders.map((item) => `- ${item.title}: ${item.body}`),
		'',
		'### Companies adopting AI',
		...capabilities.ai.map((item) => `- ${item.title}: ${item.body}`),
		'',
		'### Speaking topics',
		...capabilities.speaking.map((item) => `- ${item.title}: ${item.body}`),
		'',
		'### Questions',
